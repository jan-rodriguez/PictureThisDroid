...
fun SharedPreferences.getUser(): User? {
	val requiredKeys = listOf(USER_ID_KEY, GOOGLE_ID_KEY, USER_NAME_KEY)
	...
	requiredKeys.forEach { key ->
		if (!contains(key)) {
			...
			when (key) {
				USER_ID_KEY -> userId = getString(key, "")
	...
	return User(id = userId!!, name = name!!, google_id = googleId!!, score = score)
}
